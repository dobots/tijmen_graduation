...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
	...
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [60 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,60);
	...
	*mxGetPr(plhs[1]) = (double)exitflag;
	...
	if( nlhs > 2 )
	{
		...
		*mxGetPr(outvar) = info.QPtime;
		...
		/* QP iterations */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		*mxGetPr(outvar) = (double)info.QPit;
		mxSetField(plhs[2], 0, "QPit", outvar);
		
		/* QP exitflag */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		*mxGetPr(outvar) = (double)info.QPexitflag;
