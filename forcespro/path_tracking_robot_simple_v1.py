...
def continuous_dynamics(x, u):
    ...
    parameters:
    state x = [xPos, yPos, Vx, Vy]
    input u = [Fx, Fy]
    """
    m = 1.0
    return casadi.vertcat(  x[2],       # dxPos/dt = Vx
                            x[3],       # dyPos/dt = Vy
                            u[0] / m,   # dVx/dt = Fx/m
                            u[1] / m)   # dVy/dt = Fy/m                    
...
def generate_pathplanner():
    ...
    #                     inputs                 |  states
    #                     Fx    Fy    x      y     Vx    Vy      
    model.lb = np.array([-1.,  -2.,  -2.,   -2.,  -4., -4.])
    model.ub = np.array([+1.,  +2.,   2.,    2.,   4.,  4.])

    # Initial condition on vehicle states x
    model.xinitidx = range(2,6) # use this to specify on which variables initial conditions
    # are imposed
...
def updatePlots(x,u,pred_x,pred_u,model,k):
    ...
    ax_list[3].step(range(0, k+1), u[0, 0:k+1],'b-')         # plot new acceleration force x
    ax_list[3].step(range(k, k+model.N), pred_u[0,:],'g-')   # plot new prediction of acceleration force x
    ax_list[4].step(range(0, k+1), u[1, 0:k+1],'b-')         # plot new acceleration force y 
    ax_list[4].step(range(k, k+model.N), pred_u[1,:],'g-')   # plot new prediction of acceleration force y 
...
def main():
    ...
    x = np.zeros((4,sim_length+1)) # states
    ...
    xinit = np.transpose(np.array([0.8, 0., 0., 0.]))
    x[:,0] = xinit
    ...
    start_pred = np.reshape(problem["x0"],(6,model.N)) # first prdicition corresponds to initial guess
